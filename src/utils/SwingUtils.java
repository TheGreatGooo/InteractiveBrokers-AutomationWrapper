...
public class SwingUtils {
  ...
  public static boolean clickButton(final Window window, final String buttonText) {
    ...
    button.doClick();
    if (!button.isEnabled())
      Utils.logToConsole("Button now disabled: " + buttonText);
    return true;
  }
  ...
  /**
   * Traverses a container hierarchy and returns the ith JComboBox (0 based indexing).
   *
   * @param container the Container to search in
   * @param ith specifies which JComboBox to return (the first one is specified by 0, the next by 1,
   *        etc)
  ...
  public static JComboBox<?> findComboBox(Container container, int ith) {
    ...
    while (iter.hasNext()) {
      Component component = iter.next();
      if (component instanceof JComboBox<?> && i++ == ith)
  ...
  /**
   * Traverses a container hierarchy and returns the Component with the given text.
   * 
   * @param container the Container to search in
  ...
  /**
   * Traverses a container hierarchy and returns the radio button with the given text.
   * 
   * @param container the Container to search in
  ...
  /**
   * Traverses a container hierarchy and returns the JTextLabel that contains the given substring.
   * 
   * @param container the Container to search in
  ...
  /**
   * Traverses a container hierarchy and returns the ith JTextField (0 based indexing).
   *
   * @param container the Container to search in
   * @param ith specifies which JTextField to return (the first one is specified by 0, the next by
   *        1, etc)
  ...
  public static JTextField findTextField(Container container, int ith) {
    ...
    while (iter.hasNext()) {
      Component component = iter.next();
      if (component instanceof JTextField && i++ == ith)
        return (JTextField) component;
    }
    return null;
  }

  public static JToggleButton findToggleButton(Container container, String text) {
    ComponentIterator iter = new ComponentIterator(container);
    while (iter.hasNext()) {
      Component component = iter.next();
      if (component instanceof JToggleButton && text.equals(((JToggleButton) component).getText()))
  ...
  /**
   * Traverses a container hierarchy and returns the ith JMenuBar (0 based indexing).
   *
   * @param container the Container to search in
   * @param ith specifies which JMenuBar to return (the first one is specified by 0, the next by 1,
   *        etc)
  ...
  static JMenuBar findMenuBar(Container container, int ith) {
    ...
    while (iter.hasNext()) {
      Component component = iter.next();
      if (component instanceof JMenuBar && i++ == ith)
        return (JMenuBar) component;
    }
    return null;
  }
  ...
  /**
   * Traverses a container hierarchy and returns the JMenuItem with the given path from the first
   * JMenuBar encountered, or null if the first JMenuBar doesn't contain an item with that path
   *
   * @param container the Container to search in
   ...
   * @return true if the JButton is enabled; false if there is no such JButton, or it is disabled
   */
   ...
   * @return true if the JCheckBox is enabled; false if there is no such JCheckBox, or it is not
   *         selected
   */
   ...
   * @return true if the JRadioButton is enabled; false if there is no such JRadioButton, or it is
   *         not selected
   */
   ...
   * 
   * Details of each component in the window are included, indented to reflect the component's
   ...
   * @param value true to set the JRadioButton ; false to clear it
   ...
   * @param fieldNumber the number of the required JTextField in the window, counting from 0
  ...
  private static void appendComponentStructure(
      Component component,
    ...
    if (component instanceof JTree)
      appendTreeNodes(
          ((JTree) component).getModel(),
          ((JTree) component).getModel().getRoot(),
          builder,
          "|   " + indent);
    if (component instanceof JMenuBar) {
  ...
  private static void appendMenuSubElements(
      MenuElement element,
      StringBuilder builder,
      String indent) {
