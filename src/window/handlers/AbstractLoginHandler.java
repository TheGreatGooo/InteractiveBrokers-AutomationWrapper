...
public abstract class AbstractLoginHandler implements WindowHandler {
  ...
  public final void handleWindow(Window window, int eventID) {
    ...
    try {
      if (!initialise(window, eventID))
        return;
      if (!setFields(window, eventID))
        return;
      if (!preLogin(window, eventID))
        return;
      doLogin(window);
  ...
  protected abstract boolean initialise(final Window window, int eventID)
      throws IBControllerException;

  protected abstract boolean preLogin(final Window window, int eventID)
      throws IBControllerException;
  ...
  protected final void setCredential(
      final Window window,
      final String credentialName,
      final int credentialIndex,
      final String value) throws IBControllerException {
    if (!SwingUtils.setTextField(window, credentialIndex, value))
      throw new IBControllerException(credentialName);
  }
